...
where version_score is distinct from 'v1.2'
   or nombre_hausses_capital_12_mois < 0 or nombre_baisses_capital_12_mois < 0
   or bonus_capital_potentiel is distinct from
       (case when nombre_hausses_capital_12_mois = 0 then 0
        else 50 + 25 * (nombre_hausses_capital_12_mois - 1) end
        - 25 * nombre_baisses_capital_12_mois)
   ...
   or (segment_anciennete = 'hors_perimetre_v1' and
       (score_total is not null or couleur_priorite is not null
        or score_anciennete is not null or bonus_naf is not null or bonus_transferts is not null or bonus_capital is not null))
   or (segment_anciennete in ('lancement', 'developpement', 'renouvellement_prospection') and (
       ...
       or bonus_capital is distinct from bonus_capital_potentiel
       or score_total is distinct from score_anciennete + bonus_naf + bonus_transferts + bonus_capital
